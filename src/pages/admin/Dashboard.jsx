...
const DashboardCard = ({ title, date, count, icon }) => {
  ...
  return (
    <Card
      className={`bg-red-200 rounded-md  mb-3 ${
        isMobile ? "w-full h-60 mb-3 p-2" : "w-full p-4"
      }`}
    >
      <CardContent>
        ...
        <Typography variant={isMobile ? "h5" : "body2"} className="my-3">
          {title} : {count}
        </Typography>
...
const LineChartContainer = ({ data, dataKey, name }) => {
  ...
  return (
    <Box className="mt-8">
      ...
      <ResponsiveContainer width={isMobile ? 300 : "100%"} height={isMobile ? 200 : 300}>
        <LineChart data={data}>
...
export default function AdminDashboard() {
  ...
  const artworkIcon = <FaImages />;
  const exhibitionIcon = <FaPersonBooth />;
  const materialIcon = <FaBookReader />;
    ...
    if (Array.isArray(data)) {
      ...
      data.forEach((item) => {
        ...
        if (createdAt) {
          ...
          const itemMonth = `${itemDate.getFullYear()}-${
            itemDate.getMonth() + 1
          }`;
  ...
  return (
    <div className={`container flex ${isMobile ? "flex-col" : ""}`}>
      <Sidebar />
      <div className="flex flex-col p-5">
        ...
        <div className={`flex ${isMobile ? "flex-col" : "space-x-4"} ml-14`}>
          <DashboardCard
